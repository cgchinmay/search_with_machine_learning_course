...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        features = ["name_match", "name_match_phrase", "customerReviewAverage", "customerReviewCount", "artistName_match_phrase",
        "shortDescription_match_phrase", "longDescription_match_phrase", "salesRankShortTerm"]
        ...
        if response and len(response['hits']) > 0 and len(response['hits']['hits']) > 0:
            hits = response['hits']['hits']
            # for every query extract features
            for hit in hits:
